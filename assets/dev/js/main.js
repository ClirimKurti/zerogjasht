...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        const $tabLinkItems = $(".tabs__link-item");
    ...
    if (ads.length > 1) {
        setInterval(() => {
            $(ads[currentIndex]).animate({ opacity: 0 }, 1000);
            currentIndex = (currentIndex + 1) % ads.length;

            $(ads[currentIndex]).animate({ opacity: 1 }, 1000);
        }, 2000);
    }
    ...
    $(document).on('click', '.video-modal .play-icon, .open-modal', function (e) {
        ...
        const imgElement = $(this).hasClass('play-icon')
            ? $(this).siblings('img.open-modal')
            : $(this);

        const hdModalTitle = imgElement.data('modal-title');
        const hdModalContentType = imgElement.data('modal-content-type');
        const hdModalContent = imgElement.data('modal-content');
        ...
        if (hdModalContentType === 'youtube') {
            hdModalHTML += '<div class="modal-before">' +
                '<iframe src="https://www.youtube.com/embed/' + hdModalContent + '?autoplay=1&mute=1" ' +
                'allow="autoplay" allowfullscreen></iframe></div>';
        } else if (hdModalContentType === 'wistia') {
            hdModalHTML += '<div class="modal-before">' +
                '<iframe src="https://fast.wistia.net/embed/iframe/' + hdModalContent + '" ' +
                'allowfullscreen frameborder="0" scrolling="no" class="wistia_embed" name="wistia_embed" ' +
                'allow="autoplay; fullscreen" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; border: none;"></iframe></div>' +
                '<script src="https://fast.wistia.com/embed/medias/' + hdModalContent + '.jsonp" async></script>' +
                '<script src="https://fast.wistia.com/assets/external/E-v1.js" async></script>';
        } else if (hdModalContentType === 'mp4') {
            hdModalHTML += '<div class="modal-before">' +
                '<video controls autoplay>' +
                '<source src="' + hdModalContent + '" type="video/mp4">' +
